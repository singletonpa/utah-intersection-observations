...
# File:     funs.R
# Date:     2018 Fall, 2019 Spring, 2020 Spring, Summer, 2022 Fall
...
# 2022-09-01 fixed getsig to be more flexible of inputs
...
getsig <- function(fpath=NA, printsum=F) {
  ...
  # Rename and identify columns
  names(temp) <- toupper(names(temp))
  names(temp) <- gsub("\\.", "", names(temp))
  col1 <- which(names(temp) %in% c("SIGNALID", "DEVICEID"))
  col2 <- which(names(temp) %in% c("TIMESTAMP"))
  col3 <- which(names(temp) %in% c("EVENTCODE", "EVENTID"))
  col4 <- which(names(temp) %in% c("EVENTPARAMETER", "EVENTPARAM", "PARAMETER"))
  
  # Recode variable types (takes a LONG time to process Timestamp)
  temp$SIGNAL <- as.integer(temp[,col1])
  temp$EVENT <- as.integer(temp[,col3])
  temp$PARAM <- as.integer(temp[,col4])
  temp$TIME1 <- as.POSIXct(strptime(temp[,col2], format="%Y-%m-%d %H:%M:%OS", tz="America/Denver"))
  temp$TIME2 <- as.POSIXct(strptime(temp[,col2], format="%m/%d/%Y %H:%M:%OS", tz="America/Denver"))
  if (sum(!is.na(temp$TIME1)) > sum(!is.na(temp$TIME2))) { temp$TIME <- temp$TIME1 } else { temp$TIME <- temp$TIME2 }
